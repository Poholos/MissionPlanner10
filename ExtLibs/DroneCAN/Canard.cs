...
namespace DroneCAN
{
    public partial class DroneCAN
    {
        ...
        public delegate void dronecan_serializer_chunk_cb_ptr_t(byte[] buffer, int sizeinbits, object ctx);
 ...
 */
        ...
        public static void dronecan_transmit_chunk_handler(byte[] buffer, int bitlen, object ctx)
        {
            if (buffer == null)
            {
                ((DroneCAN.statetracking)ctx).bit += bitlen;
                return;
            }

            byte[] output = ((DroneCAN.statetracking)ctx).data;
            // check this
            var frame_bit_startofs = (((DroneCAN.statetracking)ctx).bit / (7 * 8)) * (7 * 8); // bits in a frame 
            var frame_bit_ofs = ((DroneCAN.statetracking)ctx).bit % (7 * 8);
            int chunk_bit_ofs = 0;
            ...
            ((DroneCAN.statetracking)ctx).data = output;
            ...
            ((DroneCAN.statetracking) ctx).bit += chunk_bit_ofs;
        }
