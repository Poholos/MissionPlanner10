...
#pragma warning restore CS0612
  ...
  public string GenerateSurveyGrid(double altitude, double spacing, double angle) {
    ...
    try {
      var grid = MissionPlanner.Utilities.Grid.CreateGrid(
          polygon, altitude, spacing, 0, angle, 0, 0,
          MissionPlanner.Utilities.Grid.StartPosition.Home, false, 0, 0, 0, home);
      if (grid.Count == 0) {
