...
if [[ ! "$MP_PORT_REVISION" =~ ^[0-9]+$ ]]; then
  echo "Repository build revision must be numeric: '$MP_PORT_REVISION'" >&2
  return 2 2>/dev/null || exit 2
fi
IFS=. read -r _mp_version_major _mp_version_minor _mp_version_patch <<< "$MP_UPSTREAM_VERSION"
if (( _mp_version_major > 255 || _mp_version_minor > 255 || MP_PORT_REVISION > 65535 )); then
  echo "MSI version fields exceed Windows Installer limits: $MP_UPSTREAM_VERSION / $MP_PORT_REVISION" >&2
  echo "Use a full Git checkout or set MISSIONPLANNER_PORT_REVISION to a monotonic value <= 65535." >&2
  return 2 2>/dev/null || exit 2
fi
...
MP_MSI_VERSION="$_mp_version_major.$_mp_version_minor.$MP_PORT_REVISION"
...
if [[ "${BASH_SOURCE[0]}" == "$0" ]]; then
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    ...
    "$MP_ARTIFACT_VERSION" "$MP_PORT_REVISION" "$MP_DEBIAN_VERSION" "$MP_MSI_VERSION"
fi
