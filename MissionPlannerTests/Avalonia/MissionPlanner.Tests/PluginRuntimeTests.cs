...
using HudOverlayContext = PortablePluginContract::MissionPlanner.Plugin.HudOverlayContext;
using PluginHost = PortablePluginContract::MissionPlanner.Plugin.PluginHost;
...
public sealed class PluginRuntimeTests {
  ...
  public async Task PortableDllRunsOfficialLifecycleAndCleansRegistrations() {
    ...
    Assert.DoesNotContain(AssemblyLoadContext.Default.Assemblies,
        assembly => assembly.GetName().Name == "MissionPlanner.TestPlugin.Dependency");
  ...
  public void PluginManagerIsExposedThroughOfficialShortcutAndWarningUi() {
    ...
    string xaml = File.ReadAllText(Path.Combine(
        root, "Views/MainWindow.axaml"));
    string code = File.ReadAllText(Path.Combine(
        root, "Views/MainWindow.axaml.cs"));
    string manager = File.ReadAllText(Path.Combine(
        root, "Views/PluginManagerWindow.axaml"));
  ...
  public void PortableContractRetainsOfficialLifecycleNames() {
    Type plugin = typeof(PortablePlugin);
    Assert.NotNull(plugin.GetMethod(nameof(PortablePlugin.Init)));
    Assert.NotNull(plugin.GetMethod(nameof(PortablePlugin.Loaded)));
    Assert.NotNull(plugin.GetMethod(nameof(PortablePlugin.Loop)));
    Assert.NotNull(plugin.GetMethod(nameof(PortablePlugin.Exit)));
    Assert.NotNull(plugin.GetProperty(nameof(PortablePlugin.NextRun)));
  ...
  private static string CopyFixturePlugin(string directory) {
    string source = Path.Combine(AppContext.BaseDirectory,
        "MissionPlanner.TestPlugin.dll");
    string dependency = Path.Combine(AppContext.BaseDirectory,
        "MissionPlanner.TestPlugin.Dependency.dll");
  ...
  private static string CopyLegacyFixturePlugin(string directory) {
    string source = Path.Combine(AppContext.BaseDirectory,
        "MissionPlanner.LegacyTestPlugin.dll");
